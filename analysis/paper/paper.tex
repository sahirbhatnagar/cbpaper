...
For the third case study, we will demonstrate regularization during
hazard estimation. Since we are using logistic regression, existing
packages, like \pkg{glmnet} can be leveraged. To demonstrate this
feature, we examine the Study to Understand Prognoses Preferences
...
package in \proglang{R} with default settings \citep{mice}. The imputed
dataset is available as part of the \pkg{casebase} package. For more
information please see the SUPPORT data documentation in the package.

we have a total of 9104 observations and 30 variables, two of which are
SUPPORT day 3 physiology score (sps) and APACHE III day 3 physiology
score (aps). These scores were developed for a period of 180 days, which
makes our comparison over 5.56 years exploratory. Therefore, we will
assess these two models before using either as our baseline, by
comparing them to a full model using all variables. Also, our single
imputation introduces bias in the data. As the goal here is purely
demonstrative, this bias will be consistent with any of the following
models. As such, their comparison is still valid, though any
interpretation requires an awareness of said bias. A description of used
variables can be found in Table \ref{tab:support1} and a breakdown of
each categorical variable in Table \ref{tab:support2}. The original data
was obtained from the Department of Biostatistics at Vanderbilt
University \citep{harrell_2020}.
...
beyond a Kaplan-meier curve. The original study compared these scores
over a span of 180 days, which may not be comparable over the full 5.56
years. Therefore, We will examine three cox regression scenarios: sps
only, aps only and all covariates excluding sps and aps. Next, we will
compare each modeling using the Brier score \citep{graf1999ass}. The
Brier score is able to demonstrate both discrimination and calibration
in a single model \citep{graf1999ass} and provide a comparable metric
between semi-parametric and parametric methods in the survival context.
For all the following analyses in this case study, we will use 80\% of
the observations to train our models, while 20\% will be used to
generate absolute risk curves, all of which will be averaged to
demonstrate each model's predictive ability. Over 5.56 years,
...
lowest brier score on overall, it will be used in the following absolute
risk comparison to regularized cox, regularized logistic regression and
the unadjusted Kaplan-Meier curve.
...
\caption{\label{fig:baselines} Absolute risk curves estimated from hazard models. Red: APS only hazard was used to generate the absolute risk. Green: All covariates aside from APS and SPS. Blue: SPS only. Purple: unadjusted Kaplan-Meier curve.}\label{fig:baselines}
\end{figure}
\end{CodeChunk}
...
\caption{\label{fig:brier1} Brier scores over survival time. Full model (Full) performs best, while the other two (APS only and SPS only) are comparable. Red: APS only model was used to generate the brier scores. Green: all covariates excluding APS and SPS. Blue: SPS only model.}\label{fig:brierpt1}
\end{figure}
\end{CodeChunk}

To get the absolute risk following regularized cox regression, we had to
work around limitations in existing packages. To make use of the
\pkg{survival} package, we first get our hazard coefficients using
\pkg{glmnet}'s coxnet implementation for lasso penalization
...
R> coxNet <- glmnet::cv.glmnet(x = x, y = y, family = "cox", alpha = 1, standardize = TRUE)
R> #getting all non-zero covariates
R> nonzero_covariate_cox <- predict(coxNet, type = "nonzero", s = "lambda.1se")
R> #keeping coefficients for later
R> nonzero_coef_cox <- coef(coxNet, s = "lambda.1se")
R> # cutting dataset to only include nonzero covariates
R> cleanCoxData <- as.data.frame(cbind(y, x[, nonzero_covariate_cox]))
R> #create a skeleton object
R> coxNet <- survival::coxph(Surv(time = d.time, event = death) ~ ., data = cleanCoxData,x=TRUE)
R> #replace coefficients with regularized ones
R> coxNet$coefficients <- nonzero_coef_cox@x
\end{CodeInput}
\end{CodeChunk}

For our penalized logistic regression model, we opted for the natural
log of time, which assumes a Weibull distribution for our event. As
...
R> cbFull <- casebase::fitSmoothHazard.fit(x, y, family = "glmnet",
R>                       time = "d.time", event = "death",
R>                       formula_time = ~ log(d.time), alpha = 1,
R>                       ratio = 100, standardize = TRUE, penalty.factor=c(0,rep(1,ncol(x))))
R> 
...
\begin{CodeInput}
R> 
R> cbFull <- casebase::fitSmoothHazard.fit(x, y, family = "glmnet", time = "d.time", event = "death", formula_time = ~ log(d.time), alpha = 1, ratio = 10, standardize = TRUE,penalty.factor=c(0,rep(1,ncol(x))))
\end{CodeInput}
\end{CodeChunk}
...
\caption{\label{fig:cs3lolliPlot} A lollipop plot demonstrating the estimated coefficients between the following three models: Cox (green), penalized Cox (red), penalized logisitic (blue). If a coefficient is estimated to be 0, it is removed from the plot}\label{fig:coefplots}
\end{figure}
\end{CodeChunk}
...
\caption{\label{fig:cs3Final} Four absolute risk curves, integrated from the following hazard models: Cox (green), penalized Cox (red), penalized logistic (blue) and unadjusted (Kaplan-Meier, purple).}\label{fig:unnamed-chunk-2}
\end{figure}
\end{CodeChunk}
...
intersection of cofficients with values greater than 0 for all models
are displayed in \ref{fig:cs3lolliPlot}. A visible tendency is for Cox
regression to have larger estimated coefficients. This is expected, as
there are no restrictions on the size of coefficients. The smallest
coefficient estimates are found in penalized logistic model, aside from
cancer state (cano). These values do not inform us of model performance.
To do so, we take our hazard functions and use them to predict the
absolute risk of each individual in our test set, over time. The
probabilites over time for each individual are averaged, resulting in
the absolute risk curves in figure \ref{fig:cs3Final}. There is
separation between the curves, but it is minimal. To better demonstrate
the differences between the models, we compare brier scores over time.
\ref{fig:cs3FinalBrier} the Brier score is lower in our penalized
logistic regression model, compared to the other models. This
difference, however, is minimal. We can say with relative certainty that
any of the hazard models outperform an unadjusted curve, while penalized
logistic regression performs the best on this testing dataset.
...
R+ )
R> one_yr_haz$hazard <- exp(predict(fit4,
R+   newdata = one_yr_haz,
R+   type = "link"
R+ ))
...
This report was generated on 2020-04-16 12:08:46 using the following
...
#>  [1] lubridate_1.7.8           broom_0.5.5              
#>  [3] dotwhisker_0.5.0          timereg_1.9.4            
#>  [5] glue_1.4.0                cowplot_1.0.0            
#>  [7] colorspace_1.4-1          prodlim_2019.11.13       
#>  [9] riskRegression_2020.03.27 pracma_2.2.9             
#> [11] kableExtra_1.1.0          data.table_1.12.8        
#> [13] glmnet_3.0-2              Matrix_1.2-18            
#> [15] casebase_0.2.1.9001       survival_3.1-12          
#> [17] magrittr_1.5              forcats_0.5.0            
#> [19] stringr_1.4.0             dplyr_0.8.5              
#> [21] purrr_0.3.3               readr_1.3.1              
#> [23] tidyr_1.0.2               tibble_3.0.0             
...
#> [58] mets_1.2.7.1        rlang_0.4.5         pkgconfig_2.0.3    
#> [61] evaluate_0.14       lattice_0.20-41     htmlwidgets_1.5.1  
#> [64] labeling_0.3        cmprsk_2.2-9        tidyselect_1.0.0   
#> [67] plyr_1.8.6          R6_2.4.1            generics_0.0.2     
#> [70] Hmisc_4.4-0         multcomp_1.4-13     DBI_1.1.0          
#> [73] pillar_1.4.3        haven_2.2.0         foreign_0.8-76     
#> [76] withr_2.1.2         mgcv_1.8-31         nnet_7.3-13        
#> [79] modelr_0.1.6        crayon_1.3.4        rmarkdown_2.1      
#> [82] jpeg_0.1-8.1        grid_3.6.3          readxl_1.3.1       
#> [85] reprex_0.3.0        digest_0.6.25       webshot_0.5.2      
#> [88] numDeriv_2016.8-1.1 stats4_3.6.3        munsell_0.5.0      
...
#> Head:     [7755d8f] 2020-04-09: Uses development version of Jesse-Islam/riskRegression
\end{CodeOutput}
\end{CodeChunk}
