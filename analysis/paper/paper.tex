...
of the \code{glm} function. In addition, we provide additional features,
...
\ref{tab:dists}). As discussed above, \pkg{casebase} can model any
...
explicitly model any shape parameter, unlike \pkg{flexsurv}.
...
plots; and 2) parametric modeling of the hazard function. We explicitly
aimed at being compatible with both \code{data.frame}s and
\code{data.table}s. This is evident in some of the coding choices we
made, and it is also reflected in our unit tests.
...
The splitting of these tasks gives the user the flexibility to create
population-time plots with the graphics system of their choice using the
object created in Step 1, while the object created in Step 2 allows one
to leverage the \pkg{ggplot2} \citep{ggplot2} family of functions for
...
which was extracted using the approach described in
\citep{liu2014recovering} and available through the \pkg{casebase}
package. This data contains the individual observations for 159,893 men
between the ages of 55 and 69 years at entry who were recruited from
seven European countries.

We first create the necessary dataset for producing the population time
plot using the \code{casebase::popTime} function. In this example we
stratify by the treatment group. The resulting object inherits from
class \texttt{popTime} and stores the exposure variable as an attribute:
...
We then pass this object to the corresponding \code{plot} method which
leverages the \pkg{ggplot2} package to build population time plots.
...
at the top of the y-axis. We immediately notice a unique \emph{step
shape} in the population time area which is due to the randomization of
the Finnish cohorts which were carried out on January 1 of each of the 4
years 1996 to 1999. This, coupled with the uniform December 31 2006
censoring date, lead to large numbers of men with exactly 11, 10, 9 or 8
years of follow-up. Tracked backwards in time (i.e.~from right to left),
the population-time plot shows the recruitment pattern from its
beginning in 1991, and in particular the January 1 entries in successive
...
close their recruitment - in December 2003 - the minimum potential
follow-up is three years. Tracked further forwards in time (i.e.~after
year 3) the attrition is a combination of deaths and staggered entries.
...
points along the upper edge of the plot, while the base series is
sampled horizontally with sampling weight proportional to their
follow-up time. By randomly distributing the points, we can get a better
sense of the incidence density. In the ERSPC data, we see that more
events are observed later on in time. Therefore a constant hazard model
...
The reader is referred to the package vignettes for more examples and a
detailed description of how to modify the aesthetics of a population
time plot.
...
\item
  ...
  through the \code{glmnet} package \citep{friedman2010jss}.
\item
...
We investigate the differences in survival between the control and
screening arms in the ERSPC study presented earlier using casebase
sampling to fit a parametric hazard function. We fit four models that
differ in the functional form of time; (1) excluded from the linear
predictor (Eq.\eqref{eq:exp}), (2) linear function (Eq.\eqref{eq:gomp}),
(3) log function (Eq.\eqref{eq:weibull}), and (4) a smooth function
using cubic B-splines. The models are fit using \code{fitSmoothHazard}.
Since the output object from \code{fitSmoothHazard} inherits from the
\code{glm} class, we see a familiar result when using the function
\code{summary}:
...
The \code{absoluteRisk} function takes as input the
...
casebase on the Cox model CIF. The CIF estimates for the exponential
model in panel (1) overestimate the cumulative incidence earlier on in
time, and underestimate it later on. Based on what we observed in the
population time plot, where more events are observed later on in time,
this poor fit for the exponential hazard is expected. This example
demonstrates the benefits of population time plots as an exploratory
analysis tool.

In Figure \ref{fig:erspc-cox-cif} panels (2)-(4), we notice a better fit
with increasing complexities of time. As noted above, the usual
asymptotic results hold for likelihood ratio tests built using casebase
...
#> 1     54538       6053                                    
#> 2     54535       5788  3      265 <0.0000000000000002 ***
#> ---
...
we can compare the AIC for all four casebase models:
...
hazard ratio. Of course, different parametric models for the hazards
give rise to qualitatively different estimates for the CIF as shown in
Figure \ref{fig:erspc-cox-cif}.
...
\cmidrule{1-3}
Weibull & 0.81 (0.68, 0.97) & 0.80 (0.65, 0.96)\\
\cmidrule{1-3}
...
to use this interfact, we first create a matrix \code{y} containing the
...
This report was generated on 2020-07-18 13:50:44 using the following
...
#>  [3] dplyr_0.8.5               purrr_0.3.4              
#>  [5] readr_1.3.1               tidyr_1.1.0              
#>  [7] ggplot2_3.3.2.9000        tidyverse_1.3.0          
#>  [9] lubridate_1.7.4           visreg_2.7.0             
#> [11] tibble_3.0.1              riskRegression_2020.02.05
#> [13] prodlim_2019.11.13        flexsurv_1.1.1           
#> [15] survival_3.1-8            pracma_2.2.9             
#> [17] magrittr_1.5              kableExtra_1.1.0         
#> [19] glmnet_4.0-2              Matrix_1.2-18            
#> [21] data.table_1.12.8         cowplot_1.0.0            
#> [23] colorspace_1.4-1          casebase_0.9.0           
#> 
#> loaded via a namespace (and not attached):
#>  [1] TH.data_1.0-10      VGAM_1.1-3          ellipsis_0.3.1     
#>  [4] htmlTable_1.13.3    base64enc_0.1-3     fs_1.3.2           
#>  [7] rstudioapi_0.11     farver_2.0.3        MatrixModels_0.4-1 
#> [10] fansi_0.4.1         mvtnorm_1.0-12      xml2_1.3.0         
#> [13] codetools_0.2-16    knitr_1.29          Formula_1.2-3      
...
#> [31] glue_1.4.1          Rcpp_1.0.4.6        cellranger_1.1.0   
#> [34] vctrs_0.3.1         nlme_3.1-143        iterators_1.0.12   
#> [37] xfun_0.15           rvest_0.3.5         lifecycle_0.2.0    
#> [40] polspline_1.1.17    muhaz_1.2.6.1       MASS_7.3-51.5      
#> [43] zoo_1.8-7           scales_1.1.1        hms_0.5.3          
#> [46] sandwich_2.5-1      SparseM_1.78        RColorBrewer_1.1-2 
#> [49] rticles_0.13        yaml_2.2.1          gridExtra_2.3      
#> [52] rms_5.1-4           rpart_4.1-15        latticeExtra_0.6-29
#> [55] stringi_1.4.6       foreach_1.5.0       checkmate_2.0.0    
#> [58] lava_1.6.6          shape_1.4.4         rlang_0.4.6        
#> [61] pkgconfig_2.0.3     evaluate_0.14       lattice_0.20-38    
#> [64] labeling_0.3        htmlwidgets_1.5.1   cmprsk_2.2-9       
#> [67] tidyselect_1.1.0    deSolve_1.27.1      R6_2.4.1           
#> [70] generics_0.0.2      Hmisc_4.3-1         multcomp_1.4-12    
#> [73] DBI_1.1.0           withr_2.2.0         pillar_1.4.4       
#> [76] haven_2.3.1         foreign_0.8-74      mgcv_1.8-31        
#> [79] nnet_7.3-12         mstate_0.2.12       modelr_0.1.5       
#> [82] crayon_1.3.4        rmarkdown_2.3       timereg_1.9.4      
#> [85] jpeg_0.1-8.1        grid_3.6.2          readxl_1.3.1       
#> [88] reprex_0.3.0        digest_0.6.25       webshot_0.5.2      
#> [91] numDeriv_2016.8-1.1 stats4_3.6.2        munsell_0.5.0      
...
#> Head:     [84d5ff5] 2020-06-29: merge pop-stash
\end{CodeOutput}
\end{CodeChunk}
