...
\textcolor{red}{The discussion for \pkg{CFC} discusses aspects that I'm not certain are important for our argument. Mixture models, Vertical modelling,Psuedo-observations, machine learning models. Should i include these?}
...
690 individuals.
\textcolor{red}{we lose individuals due to coliniarity. If a pair of covariates are deemed colinear to a certain degree, they are excluded from any imputations. here, only edu  income  avtisst*  race  ph*  glucose*  adlp* were imputed.}
Of these, 66.09\% experienced the event of interest. A description of
...
University
\textcolor{red}{do we need to reference the dataset itself? Or is the paper enough? In my code for generating the data in casebase, i link to the original data online and pull from there}.
The imputed dataset is available as part of the \pkg{casebase} package.
...
\ref{fig:support-poptime}. \textbackslash{}textcolor\{red\}\{red dots
now go all over. percentile\_number introduced as a parameter in
popTime\}
...
R> casebaseAbsolute=casebase::absoluteRisk(cb.Model,time = seq(1,max(completeData$d.time), 50),newdata = newData , s="lambda.1se",method=c("numerical"))
R> cab=data.frame(casebaseAbsolute[,1],rowMeans(casebaseAbsolute[,-c(1)]))
\end{CodeInput}
\end{CodeChunk}
...
but requires that a survival object skeleton of the selected variables,
so that the survival package tools can be used to retrive the absolute
...
coefficients from the regularized model will replace the coeficients in
...
R> xCox=as.matrix(sparse.model.matrix(death~ .-d.time,data=completeData))
...
R> 
R> abCoxFit<-survival::survfit(coxFit,newdata=as.data.frame(newDataCox),time = seq(0,max(completeData$d.time), 1),type="breslow")
R> 
R> abCF<-abCoxFit
R> abCF$surv<-rowMeans(abCF$surv)
\end{CodeInput}
\end{CodeChunk}
...
R> plot(cab[,1],cab[,2],type='l',col="black",lwd=3,main="Support- CaseBase vs. Cox+glmnet vs KM Absolute Risk Curves",xlab="Survival-Time",ylab="Cumulative Incidence",ylim=c(0,1),xlim=)
...
This report was generated on 2020-01-16 13:51:03 using the following
...
#>  P casebase    * 0.2.1.9001 2019-12-11 [?]
...
#> Head:     [a528002] 2019-12-12: Merge pull request #12 from sahirbhatnagar/review
\end{CodeOutput}
\end{CodeChunk}
