...
lowest brier score on overall, it will be used in the following
...
\caption{\label{fig:brier1} Brier scores over time. Full model (Cox Full) performs best, while the other two (APS only and SPS only) are comparable.}\label{fig:brierpt1}
\end{figure}
\end{CodeChunk}
...
risk of each individual in our test set, over time. The probabilites
over time for each individual are averaged, resulting in the smooth
absolute risk curve in figure \ref{fig:cs3Final}. On average, the Brier
score is lower in our penalized logistic regression model, compared to
the other models (fig.~\ref{fig:cs3FinalBrier}).

This case study serves to demonstrate how regularization can be used in
a survival context, using logistic regression. Our approach performs
comparably to Cox regression, demonstrating its validity in an applied
setting.
...
\begin{CodeInput}
R> 
R> cbFull <- casebase::fitSmoothHazard.fit(x, y, family = "glmnet", time = "d.time", event = "death", formula_time = ~ log(d.time), alpha = 1, ratio = 10, standardize = TRUE,penalty.facto=c(0,rep(1,ncol(x))))
\end{CodeInput}
\end{CodeChunk}
...
\caption{\label{fig:cs3Final} Four absolute risk curves, integrated from the following hazard models: Cox (pink), penalized Cox (green), penalized logistic (blue) and Kaplan-Meier (purple).}\label{fig:unnamed-chunk-2}
\end{figure}
\end{CodeChunk}
...
\caption{\label{fig:cs3FinalBrier} 4 curves comparing the brier scores as they change over time between a null model (Kaplan-Meier, black), Cox regression (yellow), penalized Cox regression (blue) and penalized logistic regression (green).}\label{fig:riskregressionBrier}
\end{figure}
\end{CodeChunk}
...
\caption{\label{fig:cs3FinalBrier} A lollipop plot demonstrating the estimated coefficients between the following three models: Cox (blue), penalized Cox (Green), penalized logisitic (red). If a coefficient is estimated to be 0, it is removed from the plot}\label{fig:coefplots}
\end{figure}
\end{CodeChunk}
...
This report was generated on 2020-04-09 13:28:46 using the following
...
#>  [3] prodlim_2019.11.13        riskRegression_2020.03.27
#>  [5] pracma_2.2.9              kableExtra_1.1.0         
#>  [7] data.table_1.12.8         glmnet_3.0-2             
#>  [9] Matrix_1.2-18             casebase_0.2.1.9001      
#> [11] survival_3.1-11           magrittr_1.5             
#> [13] forcats_0.5.0             stringr_1.4.0            
#> [15] dplyr_0.8.5               purrr_0.3.3              
#> [17] readr_1.3.1               tidyr_1.0.2              
#> [19] tibble_3.0.0              ggplot2_3.3.0            
#> [21] tidyverse_1.3.0          
#> 
#> loaded via a namespace (and not attached):
#>  [1] TH.data_1.0-10      VGAM_1.1-2          colorspace_1.4-1   
#>  [4] ellipsis_0.3.0      ggstance_0.3.4      htmlTable_1.13.3   
#>  [7] base64enc_0.1-3     fs_1.4.1            rstudioapi_0.11    
#> [10] farver_2.0.3        MatrixModels_0.4-1  fansi_0.4.1        
#> [13] mvtnorm_1.1-0       lubridate_1.7.8     xml2_1.3.1         
#> [16] codetools_0.2-16    knitr_1.28          Formula_1.2-3      
#> [19] jsonlite_1.6.1      cluster_2.1.0       dbplyr_1.4.2       
#> [22] png_0.1-7           compiler_3.6.3      httr_1.4.1         
#> [25] backports_1.1.6     assertthat_0.2.1    cli_2.0.2          
...
#> [37] nlme_3.1-145        iterators_1.0.12    xfun_0.12          
#> [40] rvest_0.3.5         lifecycle_0.2.0     polspline_1.1.17   
#> [43] MASS_7.3-51.5       zoo_1.8-7           scales_1.1.0       
#> [46] hms_0.5.3           sandwich_2.5-1      SparseM_1.78       
#> [49] RColorBrewer_1.1-2  rticles_0.14        yaml_2.2.1         
#> [52] gridExtra_2.3       rms_5.1-4           rpart_4.1-15       
#> [55] latticeExtra_0.6-29 stringi_1.4.6       foreach_1.5.0      
#> [58] checkmate_2.0.0     lava_1.6.7          shape_1.4.4        
#> [61] mets_1.2.7.1        rlang_0.4.5         pkgconfig_2.0.3    
#> [64] evaluate_0.14       lattice_0.20-41     htmlwidgets_1.5.1  
#> [67] labeling_0.3        cmprsk_2.2-9        tidyselect_1.0.0   
#> [70] plyr_1.8.6          R6_2.4.1            generics_0.0.2     
#> [73] Hmisc_4.4-0         multcomp_1.4-13     DBI_1.1.0          
#> [76] pillar_1.4.3        haven_2.2.0         foreign_0.8-76     
#> [79] withr_2.1.2         mgcv_1.8-31         nnet_7.3-13        
#> [82] modelr_0.1.6        crayon_1.3.4        rmarkdown_2.1      
#> [85] timereg_1.9.4       jpeg_0.1-8.1        grid_3.6.3         
#> [88] readxl_1.3.1        reprex_0.3.0        digest_0.6.25      
#> [91] webshot_0.5.2       numDeriv_2016.8-1.1 stats4_3.6.3       
...
#> Head:     [b31e195] 2020-03-31: Fixed penal. on time and Brier
\end{CodeOutput}
\end{CodeChunk}
