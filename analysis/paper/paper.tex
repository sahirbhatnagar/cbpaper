...
below, all covariates except \code{sps} and \code{aps} were modeled.
...
to the \code{fitSmoothHazard} function. We must supply both a matrix
\code{y} containing the time and event variables, and a matrix \code{x}
containing all other covariates. We applied the lasso penalty by setting
\code{alpha = 1} and assigned a \code{penalty.factor} (\(w_j\)) of 0 to
...
(fitted via the \pkg{glmnet} package).
...
used the \pkg{riskRegression} package to compute these scores for all
models.
...
This report was generated on 2020-09-20 20:42:38 using the following
...
#> [52] latticeExtra_0.6-29 stringi_1.5.3       highr_0.8          
#> [55] foreach_1.5.0       checkmate_2.0.0     lava_1.6.7         
#> [58] shape_1.4.5         rlang_0.4.7         pkgconfig_2.0.3    
...
#> [76] pillar_1.4.6        haven_2.3.1         foreign_0.8-79     
