...
to the clinician and the patient is the risk of an event given their
...
extensive array of visualization tools to complement the analysis of
time-to-event data. We illustrate all these features through three
...
purpose of the \CRANpkg{casebase} package is to provide practitioners
with an easy-to-use software tool to compute a patient's risk (or
cumulative incidence) of an event, conditional on a particular patient's
covariate profile. Our package retains the flexibility of case-base
sampling and the familiar interface of the \code{glm} function. It also
provides tools for variable selection, competing-risk analysis, and
dealing with time-dependent exposures. In addition, we provide functions
for exploratory data analysis and visualizing the estimated quantities.
...
comparable features as \CRANpkg{casebase}. Next, we provide some details
...
\CRANpkg{casebase}. We show how the same framework can be used for
...
section, we describe how the functionalities of \CRANpkg{casebase}
compare to these packages.
...
\CRANpkg{casebase}. In particular, we excluded packages that were
focused on a different set of problems, such as frailty and multi-state
models. The remaining 14 packages appear in Table \ref{tab:surv-pkgs},
along with some of the functionalities they offer.
...
\citeyearpar{smoothHazard}, \CRANpkg{rsptm2} \citeyearpar{clements_liu},
\CRANpkg{mets} \citeyearpar{scheike2014estimating}, and
\CRANpkg{survival} \citeyearpar{survival-package}. For example,
\CRANpkg{SmoothHazard} is limited to Weibull distributions
\citeyearpar{smoothHazard}, whereas both \CRANpkg{flexsurv} and
\CRANpkg{survival} allow users to supply any distribution of their
choice. \CRANpkg{flexsurv}, \CRANpkg{smoothhazard}, \CRANpkg{mets} and
\CRANpkg{rstpm2} can model the effect of time using splines, which
allows flexible modeling of the hazard function. As discussed above,
\CRANpkg{casebase} can model any parametric family whose log-hazard can
be expressed as a linear combination of covariates (including time).
...
\CRANpkg{flexsurv}, we do not explicitly model any shape parameter.
...
\CRANpkg{rstpm2} \citeyearpar{clements_liu}, while \CRANpkg{survival}
\citeyearpar{survival-package} has an implementation of ridge-penalized
estimation. On the other hand, our package \CRANpkg{casebase} provides
penalized estimation of the hazard function. To our knowledge,
\CRANpkg{casebase} and \CRANpkg{rsptm2} are the only packages to offer
this functionality.
...
\CRANpkg{CFC} estimates cause-specific CIFs from unadjusted,
non-parametric survival functions. Our package \CRANpkg{casebase} also
provides functionalities for competing risk analysis by estimating
parametrically the cause-specific hazards. From these quantities, we can
then estimate the cause-specific CIFs.
...
below). Packages providing this functionality include \CRANpkg{CFC},
\CRANpkg{flexsurv}, \CRANpkg{mets}, and \CRANpkg{survival}. Our package
\CRANpkg{casebase} also follows this approach for both single-event and
...
this approach include \CRANpkg{riskRegression}, \CRANpkg{rstpm2}, and
\CRANpkg{survival}. As mentioned in the introduction, a key
distinguishing factor between these two approaches is that the first
category leads to smooth estimates of the cumulative incidence function,
whereas the second category often produces estimates in the form of
stepwise functions. Providing smooth estimates of the CIF was one of the
main motivations for introducing case-base sampling in survival
analysis.
...
The functions in the \CRANpkg{casebase} package can be divided into two
...
dataset is available through the \CRANpkg{casebase} package. It contains
the individual observations for 159,893 men from seven European
...
\item
  ...
  This functionality is provided through the \CRANpkg{glmnet} package
...
\CRANpkg{casebase} package through three case studies. Each one
showcases a different type of analysis. First, we show how to model
different functional forms of time as well as non-proportional hazards.
Then we perform a competing-risk analysis and compare our results with
the Fine-Gray model. The third case study illustrates how to perform
variable selection in high-dimensional datasets.
...
\CRANpkg{casebase} on the Cox model CIF. The CIF estimates for the
...
\CRANpkg{casebase} package by including an interaction term with time in
the model:
...
For the third case study, we show how \CRANpkg{casebase} can also be
used for variable selection through regularized estimation of the hazard
...
American hospitals within individuals who are considered seriously ill.
The original data is available online from the Department of
...
is available as part of the \CRANpkg{casebase} package. In the
comparisons below, all covariates except \code{sps} and \code{aps} were
used. These two variables correspond to scores for predicting the
outcome that were developed as part of the original study. For more
information about this dataset, the reader is encouraged to look at the
documentation in our package.
...
and lasso penalized Cox regression (fitted via the \CRANpkg{glmnet}
package).
...
use the \CRANpkg{riskRegression} package to compute these scores for all
models.
...
In this article, we presented the \textsf{R} package \CRANpkg{casebase},
which provides functions for fitting smooth parametric hazards and
estimating CIFs using case-base sampling. Our package also provides
...
sampling framework in general, and the package \CRANpkg{casebase} in
...
function interface, \CRANpkg{casebase} uses a formula interface that is
...
nature of the output of \CRANpkg{casebase} provides a more intuitive
...
parameters separately like the package \CRANpkg{flexsurv}. For example,
if we look at the Weibull distribution as parametrised in
...
Unlike \CRANpkg{casebase}, the approach taken by \CRANpkg{flexsurv} also
allows the user to model the scale parameter as a function of
covariates. Of course, this added flexibility comes at the cost of
interpretability: by modeling the log-hazard directly, the parameter
estimates from \CRANpkg{casebase} can be interpreted as estimates of
log-hazard ratios. To improve the flexibility of \CRANpkg{casebase} at
capturing the scale of a parametric family, we could replace the
logistic regression with its quasi-likelihood counterpart and therefore
model over- and under-dispersion with respect to the logistic
likelihood. We defer the study of the properties and performance of such
a model to a future article.
...
longitudinal studies, and many packages (e.g. \CRANpkg{SmoothHazard},
\CRANpkg{survival} and \CRANpkg{rstpm2}) provide functions to account
for it. Again, we hope to include any resulting methodology as part of
this package.
...
potentially be included in \CRANpkg{casebase}. Finally, we want to
provide more flexibility in how the case-base sampling is performed.
This could be achieved by adding a \code{hazard} argument to the
function \code{sampleCaseBase}. In this way, users could specify their
own sampling mechanism. For example, they could provide a hazard that
gives sampling probabilities that are proportional to the cardiovascular
disease event rate given by the Framingham score \citep{saarela2015non}.

In conclusion, we presented the \textsf{R} package \CRANpkg{casebase}
...
\CRANpkg{casebase} package is freely available from the Comprehensive
\textsf{R} Archive Network at
