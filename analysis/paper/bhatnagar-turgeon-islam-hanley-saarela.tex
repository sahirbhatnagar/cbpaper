...
approach. This leads to step-wise estimates of the survival function
...
estimates of the survival function that are smooth-in-time.\\
...
Figure \ref{fig:plot-erspc-data} is built sequentially by first adding a
layer for the area representing the population time in gray (Figure
...
series is sampled horizontally with sampling weight proportional to
their follow-up time (Figure \ref{fig:plot-erspc-data} D). The reader
should refer to the package vignettes for more examples and a detailed
description of how to modify the aesthetics of a population-time plot.
...
#> -0.268  -0.171  -0.133  -0.091   3.623  
#> 
#> Coefficients:
#>                       Estimate Std. Error z value Pr(>|z|)    
#> (Intercept)            -10.824      0.503  -21.52   <2e-16 ***
#> bs(Follow.Up.Time)1      2.639      1.359    1.94   0.0523 .  
#> bs(Follow.Up.Time)2      2.479      0.951    2.61   0.0092 ** 
#> bs(Follow.Up.Time)3      3.063      1.354    2.26   0.0237 *  
#> ScrArmScreening group   -0.207      0.161   -1.29   0.1986    
#> ---
#> Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
#> 
#> (Dispersion parameter for binomial family taken to be 1)
#> 
#>     Null deviance: 1817.7  on 16361  degrees of freedom
#> Residual deviance: 1750.8  on 16357  degrees of freedom
#> AIC: 1761
#> 
...
In Figure \ref{fig:erspc-cox-cif}, we overlay the estimated risk
function from \pkg{casebase} on the Cox model estimate. As noted in the
...
#> NULL                               16361       1818             
#> bs(Follow.Up.Time)  3     65.2     16358       1752  4.5e-14 ***
#> ScrArm              1      1.7     16357       1751      0.2    
#> ---
#> Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
\end{Soutput}
\end{Schunk}
...
\begin{Schunk}
\begin{figure}[ht]
\includegraphics[width=\textwidth,keepaspectratio=true]{./erspc-cox-cif-1} \caption{Risk functions for control and screening groups in the ERSPC data. We plot the estimate from the Cox model using \code{survival::survfit} (solid line) and the estimate from case-base sampling (dashed line). We used a cubic B-spline expansion of time.}\label{fig:erspc-cox-cif}
\end{figure}
\end{Schunk}

Moreover, we can also compute confidence intervals for the risk function
...
#> 2    1  2.6e-05  1.3e-05   5.7e-05   Control group
#> 3    0  0.0e+00  0.0e+00   0.0e+00 Screening group
#> 4    1  2.1e-05  9.6e-06   4.4e-05 Screening group
\end{Soutput}
\end{Schunk}
...
the function \code{glmnet::cv.glmnet} and the binomial family. We supply
both a matrix \code{y} containing the time and event variables, and a
matrix \code{x} containing all other covariates. We apply the lasso
penalty by setting \code{alpha = 1} and assign a \code{penalty.factor}
...
\pkg{glmnet} package and using the Cox family).
...
the output of \pkg{casebase} provides a more intuitive interpretation
