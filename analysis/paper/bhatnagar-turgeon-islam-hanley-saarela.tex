...
\abstract{%
In clinical studies of time-to-event data, a quantity of interest to the
clinician is their patient's risk of an event. However, methods relying
on time matching or risk-set sampling (including Cox regression)
...
survival or cumulative incidence curves. Indeed, reporting patient risk
or cumulative incidence requires a separate estimation of the baseline
hazard. Using case-base sampling, Hanley \& Miettinen (2009) explained
how parametric hazard functions can be estimated in continuous-time
using logistic regression. Their approach naturally leads to estimates
of the survival or risk function that are smooth-in-time.\\
...
the familiar interface of the \code{glm} function. Our implementation
...
In what follows, we first recap some theoretical details on case-base
...
features to \pkg{casebase}. Next, we provide some details about the
...
Therefore, an estimating function for \(\theta\) can be composed of
these contributions as: \begin{equation}
...
valid when \code{family = "glm"} as it relies on the asymptotic
