...
experiencing a certain event given the patient's particular circumstances. 
Unfortunately, the most reported metric from a Cox model is the (potentially 
time-dependent) hazard ratio (HR), which ignores the duration of follow-up or 
is subject to selection bias [@hernan2010hazards]. The covariate adjusted 
survival curve overcomes these limitations, and is arguably a more important 
...
fitting of smooth hazard functions much more transparent and intuitive through 
generalized linear models. The key feature of these procedures is splitting the 
time axis into discrete intervals. [@whitehead1980fitting] showed the 
equivalence between a Cox model and a Poisson regression with a parameter for 
each event time. [@copenhagen2012needs] provides a nice exposition of this 
equivalence with a real data example and supporting R code for computing 
standard errors. [@arjas1987logistic] and [@efron1988logistic] treated each 
...
number of time bins need to be chosen by the data analyst. A fine grouping 
...
[@kalbfleisch2011statistical]. A coarse grouping could potentially mask 
...
study base to obtain the logistic form likelihood expression for the hazard 
function. More specifically, all person-moments when the event of interest 
...
flexible modeling of the hazard function by including time as a covariate 
(e.g. using splines or general additive models). Furthermore, time-dependent 
...
for directly modeling the hazard function, but do not require an arbitrary 
choice of splitting time. 

In this article, we present the \CRANpkg{casebase} R package [@casebase-package] 
which implements the Hanley & Miettinen [-@hanley2009fitting] approach for 
fitting fully parametric hazard models and covariate adjusted survival curves 
using the familiar interface of the \code{glm} function. Our implementation 
allows for straightforward extensions to other models such as penalized 
...
quantities such as the hazard function, survival curve and its standard errors. 
The ultimate goal of our package is to make fitting flexible hazards accessible 
to more endusers with the hopes that they will favor reporting absolute risks 
over hazard ratios. 

In what follows, we first recall some theoretical details on case-base sampling 
and its use for estimating parametric hazard functions. We then give a short 
review of existing R packages that implement comparable features as \pkg{casebase}. 
...
toRemove<-sample(which(ERSPC$DeadOfPrCa==1),round(length(which(ERSPC$DeadOfPrCa==1))*0.7))
ERSPC$DeadOfPrCa[toRemove]<-0

a <- casebase::popTime(ERSPC, time = "Follow.Up.Time",
                               event = "DeadOfPrCa")

plota<-emptyPlot<-plot(a,add.case.series=F,legend=F) +scale_y_continuous(labels = label_number_si())

plotb<-emptyPlot+geom_point(aes(x=a$time,y=a$ycoord, color = "Case series", fill = "Case series"), show.legend = F, size = 1.5, alpha = 0.5, shape = 21)+ theme(legend.position="none") +scale_y_continuous(labels = label_number_si())

plotc<-plot(a,add.case.series=T,legend=F) +scale_y_continuous(labels = label_number_si())

#TEMP MAINLY NEED FOR LEGEND
plotd<-plot(a,add.base.series=T) +scale_y_continuous(labels = label_number_si())
...
plotspop<-cowplot::plot_grid(plota,plotb,plotc,plotd,labels=c("A","B","C","D"))
...
\textcolor{red}{Poisson regression can be used to estimate the full hazard. However, this method requires the user to decide on the number of intervals. Too fine the intervals may result in many empty bins. This may cause the commonly used Newton-Raphson procedure to fail to converge [@kalbfleisch2011statistical]. If the intervals are too wide, the nonlinear trends that are present in the hazard may be masked. Rather than discretizing time like in Poisson regression, we sample person-specific moments which have been experienced during the study.}
