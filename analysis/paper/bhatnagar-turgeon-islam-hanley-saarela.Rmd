...
[@kalbfleisch2011statistical]. On the other hand, a coarse grouping could potentially mask 
...
Next, we can compute confidence intervals for the risk function using the method \code{confint.absRiskCB}. 
...
Previous re-analyses of these data suggest that the 20% reduction in prostate cancer death due to screening was an underestimate [@hanley2010mortality]. The estimated 20% (from a proportional hazards model) did not account for the delay between screening and the time the effect is expected to be observed. As a result, the null effects in years 1–7 masked the substantial reductions that began to appear from year 8 onward. This motivates the use of a time-dependent hazard ratio which can easily be fit with the \pkg{casebase} package by including an interaction term with time in the model:
...
Note that we did not have to specify any cut points, as would be the case with the \code{survSplit} function in the \pkg{survival} package. In Figure \ref{fig:interaction-ERSPC}, we have the estimated hazard ratio and 95% confidence interval for screening vs. control group as a function of time using the \code{plot} method for objects of class \code{singleEventCB}:
