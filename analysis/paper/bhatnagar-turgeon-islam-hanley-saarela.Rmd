...
In Figure \ref{fig:erspc-cox-cif}, we overlay the estimated CIF from \pkg{casebase} on the Cox model CIF. As noted in the Theoretical Details section, the usual asymptotic results hold for likelihood ratio tests built using case-base sampling models. Therefore, we can easily test the significance of the spline model:
...
Moreover, we can also compute confidence intervals for the CIF using the method \code{confint.absRiskCB}. 
...
These point-wise confidence intervals are constructed using parametric bootstrap; the validity of this approach relies on the asymptotic normality of the MLE. In Figure \ref{fig:erspc-cif-conf}, we see the 95% confidence bands around the CIF estimates.

```{r erspc-cif-conf, eval = eval_cs1, fig.align='h', echo = FALSE, fig.width=8, fig.height=6, fig.cap="CIF estimates for control and screening groups in the ERSPC data using case-base sampling and Kaplan-Meier, along with 95\\% confidence bands."}
# confidence interval
