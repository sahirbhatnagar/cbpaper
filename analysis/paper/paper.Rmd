...
reg_ci <- ggplot(ab, aes(Time)) +
  ...
  scale_color_manual(values = q4, breaks = c("K-M","Cox","Penalized Cox","Penalized logistic")) + 
  paper_gg_theme
```
...
```{r riskregressionBrier, echo = FALSE, eval = eval_cs3, fig.cap="\\label{fig:cs3FinalBrier} (A) Four curves comparing the probabilities of death as it changes over follow-up time between Cox regression, penalized Cox regression, penalized logistic regression, and Kaplan-Meier. All follow-up times seen in the new data were used to generate the penalized casebase and K-M curves. The Cox-based curves, estimated using the survival package, appear to be smooth because all 1703 follow-up times seen while fitting are used. Using user specified times is not permitted in the survival package. (B) Four curves demonstrating the Brier scores as they change over time between Cox regression, penalized Cox regression, penalized logistic regression, and Kaplan-Meier. Lower is better. Penalized logistic regression produced the lowest Brier scores over follow-up time.", warning = FALSE}
...
data_brier$model<-factor(data_brier$model,c("Null model", "PredictionModel","Penalized Cox", "Penalized Logistic"))
levels(data_brier$model)<-c("Kaplan-Meier","Cox","Penalized Cox","Penalized logistic")
