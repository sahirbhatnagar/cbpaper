...
The purpose of the next case study is to show how \pkg{casebase} can also be used for survival analysis problems that do not share these properties. To this end, we will use the Stanford Heart Transplant Data [@clark1971cardiac,@crowley1977covariance]; we will use the version available in the \pkg{survival} package. 
...
Note that the fourth model (i.e. `fit4`) includes an interaction term between exposure and follow-up time. In other words, this model no longer exhibit proportional hazards. We can then compare the goodness of fit of these four models using the Akaike Information Criterion (AIC).
