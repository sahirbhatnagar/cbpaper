...
\begin{equation}
\min _{\theta \in \mathbb{R}^{p}}\,\,-\ell\left(\theta\right)+\sum_{j=1}^p w_j P(\theta_j;\lambda,\alpha)
\end{equation}
...
To illustrate this functionality, we use the dataset from the Study to Understand Prognoses Preferences Outcomes and Risks of Treatment (SUPPORT) [@knaus1995support]. The SUPPORT dataset tracks death in five American hospitals within individuals who are considered seriously ill. The original data is available online from the Department of Biostatistics at Vanderbilt University [@harrell_2020]. The cleaned and imputed data consists of 9104 observations and 30 variables, and it is available as part of the \pkg{casebase} package. For more information about this dataset, the reader is encouraged to look at the documentation in our package.

In this section, we compare our penalized case-base approach to two other models: Cox regression, and penalized Cox regression (using the \pkg{glmnet} package). In all three models, we use all covariates except \code{sps} and \code{aps}. These two variables correspond to scores for predicting the outcome that were developed as part of the original study.

To compare the performance of our models, we first divide the data into a training and a test data, with 95% of the observations used for training all three models. To assess both discrimination and calibration, we use a time-dependent version of the classical Brier score that is adjusted for censoring [@graf1999ass]. The Brier score can be used with both parametric and semi-parametric models. We use the \pkg{riskRegression} to compute these scores for all models.
...
For our penalized case-base model, we opt for the natural log of time; recall that this corresponds to a Weibull distribution. For fitting the penalized hazard, we use \code{fitSmoothHazard.fit}, which is a matrix interface to the function \code{fitSmoothHazard}. In order to use this interface, we first create a matrix \code{y} containing the time and event variables, and a matrix \code{x} containing all remaining covariates we would like to include in the model space. We used lasso penalization by setting \code{alpha = 1}. Moreover, we use the argument \code{penalty.factor} to give a regularization weight of 0 to the time variable, and therefore force it into the final model.
...
Our first comparison plot examines the covariates that were selected between all three hazard functions. In \ref{fig:cs3lolliPlot}, we show the coefficient estimates for covariates that we selected by both penalized Cox and penalized case-base. From this plot, we can clearly see the shrinkage effect of using penalized estimation. 
...
In brief, case-base sampling can be used in conjunction with penalized logistic regression to perform variable selection with survival models.
...
We would like to thank Yi Yang for helpful discussions on penalized regression models. \mbox{Bhatnagar} gratefully acknowledges funding via Discovery Grants from the Natural Sciences and Engineering Research Council of Canada (NSERC).
