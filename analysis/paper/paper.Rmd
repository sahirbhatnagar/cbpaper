...
In this article, we present an \proglang{R} package that combines the ideas of Hanley & Miettinen into a simple interface. The purpose of the \pkg{casebase} package is to provide practitioners with an easy-to-use software tool to predict the risk (or cumulative incidence) of an event, conditional on a particular patient's covariate profile. Our package retains the flexibility of case-base sampling and the familiar interface of the \code{glm} function. In addition, we provide additional features, including performance metrics and visualization tools.
...
When a logarithmic link function is used for modelling the hazard function, the above expression is of a logistic regression form with an offset term $\log(1/\rho_i(t))$. Note that the sampling units selected in the case-base sampling mechanism are person-moments, rather than individuals, and the parameters to be estimated are hazards or hazard ratios rather than odds or odds ratios. Generally, an individual can contribute more than one person-moment, and thus the terms in the product integral are not independent. Nonetheless, Saarela [-@saarela2016case] showed that the logarithm of this estimating function has mean zero at the true value $\theta=\theta_0$, and that the resulting estimator $\hat{\theta}$ is asymptotically normally distributed.
...
The functions in the casebase package can be divided into two categories: 1) exploratory data analysis, in the form of population-time plots; and 2) parametric modeling of the hazard function. We explicitly aimed at being compatible with both \code{data.frame}s and \code{data.table}s. This is evident in some of the coding choices we made, and it is also reflected in our unit tests.

## Population-time plots

The case-base sampling approach described in Section \ref{theory} can be visualized in the form of a population time plot. They are extremely informative graphical displays of survival data and should be one of the first steps in an exploratory data analysis. We facilitate this task in the \pkg{casebase} package using the \code{popTime} function and \code{plot} method as follows:
...
The splitting of these tasks gives the user the flexibility to create population-time plots with the graphics system of their choice using the object created in Step 1, while the object created in Step 2 allows one to leverage the \pkg{ggplot2} [@ggplot2] family of functions for further customization.

To illustrate these functions we will use the European Randomized Study of Prostate Cancer Screening (ERSPC) data [@schroder2009screening] which was extracted using the approach described in [@liu2014recovering] and available through the \pkg{casebase} package. This data contains the individual observations for 159,893 men between the ages of 55 and 69 years at entry who were recruited from seven European countries. 

We first create the necessary dataset for producing the population time plot using the \code{casebase::popTime} function. In this example we stratify by the treatment group. The resulting object inherits from class `popTime` and stores the exposure variable as an attribute:
...
We then pass this object to the corresponding \code{plot} method which leverages the \pkg{ggplot2} package to build population time plots. 
```{r plot-stratified-erspc-data-code, eval = FALSE, echo = TRUE}
plot(pt_object, add.base.series = TRUE)
```

```{r plot-stratified-erspc-data, eval = eval_cs1, echo = FALSE}
plot(pt_object, 
     add.base.series = TRUE,
     ratio = 0.5,
     facet.params = list(ncol = 2),
     ribbon.params = list(fill = "gray30"),
     case.params = list(size = 0.5),
     base.params = list(size = 0.5)) + theme_bw(base_size = 10) + theme(legend.position = "bottom") + 
  scale_y_continuous(labels = scales::label_number_si())
```


Figure \ref{fig:plot-stratified-erspc-data} is built sequentially by first adding a layer for the area representing the population time in gray, with subjects having the least amount of observation time plotted at the top of the y-axis. We immediately notice a unique _step shape_ in the population time area which is due to the randomization of the Finnish cohorts which were carried out on January 1 of each of the 4 years 1996 to 1999. This, coupled with the uniform December 31 2006 censoring date, lead to large numbers of men with exactly 11, 10, 9 or 8 years of follow-up. Tracked backwards in time (i.e. from right to left), the population-time plot shows the recruitment pattern from its beginning in 1991, and in particular the January 1 entries in successive years. Tracked forwards in time (i.e. from left to right), the plot for the first three years shows attrition due entirely to death (mainly from other causes). Since the Swedish and Belgian centres were the last to close their recruitment - in December 2003 - the minimum potential follow-up is three years. Tracked further forwards in time (i.e. after year 3) the attrition is a combination of deaths and staggered entries. 

Next, layers for the case series and base series are added. The case series is sampled at random vertically on the plot to avoid having all points along the upper edge of the plot, while the base series is sampled horizontally with sampling weight proportional to their follow-up time. By randomly distributing the points, we can get a better sense of the incidence density. In the ERSPC data, we see that more events are observed later on in time. Therefore a constant hazard model would not be appropriate in this instance as it would overestimate the cumulative incidence earlier on in time, and underestimate it later on. The reader is referred to the package vignettes for more examples and a detailed description of how to modify the aesthetics of a population time plot.
...
To fit the hazard function, we provide several approaches that are available via the \code{family} parameter. These approaches are:
  ...
  - \code{glmnet}: This option allows for variable selection using Lasso or elastic-net (cf. Case Study 3). This functionality is provided through the \code{glmnet} package [@friedman2010jss].
...
We investigate the differences in survival between the control and screening arms in the ERSPC study presented earlier using casebase sampling to fit a parametric hazard function. We fit four models that differ in the functional form of time; (1) excluded from the linear predictor (Eq.\eqref{eq:exp}), (2) linear function (Eq.\eqref{eq:gomp}), (3) log function (Eq.\eqref{eq:weibull}), and (4) a smooth function using cubic B-splines. The models are fit using \code{fitSmoothHazard}. Since the output object from \code{fitSmoothHazard} inherits from the \code{glm} class, we see a familiar result when using the function \code{summary}:
...
The \code{absoluteRisk} function takes as input the \code{fitSmoothHazard} object and returns a matrix where each column corresponds to the covariate profiles specified in the \code{newdata} argument, and each row corresponds to a specified time point:
...
In Figure \ref{fig:erspc-cox-cif}, we overlay the estimated CIFs from casebase on the Cox model CIF. The CIF estimates for the exponential model in panel (1) overestimate the cumulative incidence earlier on in time, and underestimate it later on. Based on what we observed in the population time plot, where more events are observed later on in time, this poor fit for the exponential hazard is expected. This example demonstrates the benefits of population time plots as an exploratory analysis tool. 

In Figure \ref{fig:erspc-cox-cif} panels (2)-(4), we notice a better fit with increasing complexities of time. As noted above, the usual asymptotic results hold for likelihood ratio tests built using casebase sampling models. Therefore, we can easily test the null hypothesis that the exponential model is just as good as the larger (in terms of number of parameters) splines model:
...
As expected, we see that splines model provides a better fit. Similarly, we can compare the AIC for all four casebase models:
...
```{r erspc-cox-cif, eval = eval_cs1, echo = FALSE, fig.width=8, fig.height=6, fig.cap="Cumulative incidence functions for control and screening groups in the ERSPC data. In each of the panels, we plot the CIF from the Cox model using \\code{survival::survfit} (solid line) and the CIF from the caseabse sampling scheme (dashed line) with different functional forms of time in a logistic regression. (1) The time variable is excluded so that the hazard function is constant over time which is equivalent to the one-parameter exponential distribution. (2) Linear function of time which is equivalent to a Gompertz distribution. (3) The natural logarithm of time which is equivalent to a Weibull distribution. (4) Cubic B-spline expansion of time."}
...
exp_est_ci <- function(model = c("exponential","gompertz","weibull","splines"),
  ...
  if (type == "casebase"){
    ...
    if (model == "weibull") {
      ...
      # The maximised log-likelihoods are the same between survreg and flexsurvreg, 
...
For our penalized logistic regression model, we opt for the natural log of time; recall that this corresponds to a Weibull distribution. For fitting the penalized hazard, we use \code{fitSmoothHazard.fit}, which is a matrix interface to the function \code{fitSmoothHazard}. In order to use this interfact, we first create a matrix \code{y} containing the time and event variables, and a matrix \code{x} containing all remaining covariates we would like to include in the model space. We used lasso penalization by setting \code{alpha = 1}. Moreover, we use the argument \code{penalty.factor} to force the time variable into the final model. 
...
cowplot::plot_grid(plot_row_reg, legend_reg, rel_widths = c(3, .4))
```
...
colnames(jasa)[which(colnames(jasa)=="futime")] <- "futime_old"
colnames(jasa)[which(colnames(jasa)=="age")] <- "futime"
...
THIS NEXT CODE CHUNK IS NOT WORKING

```{r stanford-hazard, eval = FALSE, echo=TRUE}
# Compute hazards---
