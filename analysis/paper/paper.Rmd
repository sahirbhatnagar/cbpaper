...
class(cbFull) <- c("singleRisk",class(cbFull))
brierFinalResults <- Score(list("Cox" = coxFull,"Penalized Cox"=coxNet,"Penalized Logistic"=cbFull), data = test, formula = Hist(time, event != 0) ~ 1, summary = "ipa", se.fit = 0L, metrics = "brier", contrasts = FALSE, times =times)
...
```{r coefplots,echo=FALSE, eval=eval_cs3}
