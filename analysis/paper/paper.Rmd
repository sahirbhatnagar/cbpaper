...
xInteractions=x*y[,2]
colnames(xInteractions) <- paste("d.time*", colnames(xInteractions), sep = "")
xFull=cbind(x,xInteractions)
cb.ModelInter=casebase::fitSmoothHazard.fit(xFull,y,family="glmnet",time="d.time",event="death",
                                       formula_time = ~d.time,alpha=1,ratio=rat)
```
...
casebaseAbsoluteInter=casebase::absoluteRisk(cb.ModelInter,time = seq(1,max(completeData$d.time), 400),newdata = xFull , s="lambda.1se",method=c("numerical"))

cabI=data.frame(casebaseAbsoluteInter[,1],rowMeans(casebaseAbsoluteInter[,-c(1)]))
```
...
```{r coxHazAbsoluteInteracts, echo = TRUE, eval = eval_cs3}
#Create u and xCox, which will be used when fitting Cox with glmnet.
u=survival::Surv(time = as.numeric(y[,2]), event = as.factor(y[,1]))

xCoxI=as.matrix(cbind(data.frame("(Intercept)"=rep(1,each=690)),xFull))
colnames(xCoxI)[1]="(Intercept)"

#hazard for cox using glmnet
coxNetI=glmnet::cv.glmnet(x=xCoxI,y=u, family="cox",alpha=1)
#convergence demonstrated in plot
#plot(coxNet)
#taking the coefficient estimates for later use
nonzero_covariate_cox <- predict(coxNetI, type = "nonzero", s = "lambda.1se")
nonzero_coef_cox <- coef(coxNetI, s = "lambda.1se")
...
cleanCoxDataI<-as.data.frame(cbind(as.numeric(y[,2]),as.numeric(y[,1]),xCoxI[,nonzero_covariate_cox$X1]))
...
coxNetFitI <- survival::coxph(Surv(time=V1,event=V2) ~ ., data = cleanCoxDataI)
...
coxNetFitI$coefficients<-nonzero_coef_cox@x
...
abcoxNetI<-survival::survfit(coxNetFitI,type="breslow",newdata=cleanCoxDataI)

abCNI<-abcoxNetI
abCNI$surv=rowMeans(abCNI$surv)
```


```{r unpenalizedCox, echo = TRUE, eval = eval_cs3}
fullDataCox<-as.data.frame(cbind(as.numeric(y[,2]),as.numeric(y[,1]),xCoxI[,-c(1)]))
coxFitI <- survival::coxph(Surv(time=V1,event=V2) ~ ., data = fullDataCox)
abcoxFitI<-survival::survfit(coxFitI,newdata=fullDataCox)
abCFI<-abcoxFitI
abCFI$surv<-rowMeans(abCFI$surv)
#cb.ModelNorm=casebase::fitSmoothHazard(V2~.,time="V1",event="V2",
       #                                ratio=rat,data=fullDataCox)
#summary(cb.ModelNorm)#need to figure out how to deal with NAs
#casebaseAbsoluteNorm=casebase::absoluteRisk(cb.ModelNorm,time = seq(1,max(completeData$d.time), 200),newdata = newData , s="lambda.1se",method=c("numerical"))
#cabn=data.frame(casebaseAbsoluteNorm[,1],rowMeans(casebaseAbsoluteNorm[,-c(1)]))
...
coxFitup <- survival::coxph(Surv(time=V1,event=V2) ~ ., data = Cox)
abcoxFitup<-survival::survfit(coxFitup,newdata=Cox)
...
Now that our four cumulative incidence estimations have been calculated, we compare them all in Figure \ref{fig:abSC}. Both coxnet and casebase decrease the absolute risk by the end of the study, in comparison to kaplan-meier.
```{r abSupportComparison, echo = FALSE, eval = eval_cs3, fig.cap="\\label{fig:abSC} Compares regularized casebase in black, regularized cox in red and Kaplan meier in blue."}
abCFup<-abcoxFitup
abCFup$surv<-rowMeans(abCFup$surv)
# A plot to compare all three Absolute risk (Commulative Incidence)
plot(cabI[,1],cabI[,2],type='l',col="black",lwd=3,main="Support- CaseBase vs. Cox+glmnet vs KM Absolute Risk Curves",xlab="Survival-Time",ylab="Cumulative Incidence",ylim=c(0,1),xlim=)
lines(abCFI,col="red",fun="event",lwd=3,conf.int = FALSE)
lines(abKm ,col="Blue",fun="event",lwd=3,conf.int = FALSE)
lines(abCNI,col="purple",fun="event",lwd=3,conf.int = FALSE)
legend("bottomright", 
       legend = c( "Casebase (Lasso+linear)","semi-parametric Coxnet","KM curve","cox"), 
...
```{r covariatesLolliInteractions,eval=eval_cs3}
#get covariates excluding d.time
library(data.table)
estimatescb=setDT(as.data.frame(coef(cb.ModelInter)[-c(1,2),1]), keep.rownames = TRUE)[]
estimatescb$Model="casebase"
colnames(estimatescb)<-c("Coefs","Estimate","Model")
estimatescox=setDT(as.data.frame(coef(coxNetI)[-c(1),1]), keep.rownames = TRUE)[]
estimatescox$Model="Cox"
colnames(estimatescox)<-c("Coefs","Estimate","Model")
estimatesI=rbind(estimatescb, estimatescox)
estimatesI$Estimate[estimatesI$Estimate==0]=NA# make it so unchosen covariates appear empty
library(ggstance)# for vertical dodge
  ggplot(estimatesI, aes(x = Coefs, y = Estimate , color = Model)) +
        geom_segment(aes(y = 0, x = Coefs, yend = Estimate, xend = Coefs),size=0.5, position = position_dodge(width = 0.5)) +
