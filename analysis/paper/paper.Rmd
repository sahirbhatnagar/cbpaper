...
In the first two case studies, we described the basic functionalities of the \pkg{casebase} package: creating population-time plots, fitting parametric models for hazard functions, and estimating the corresponding cumulative incidence curves. In the next two case studies, we explore advanced features.

For the third case study, we will demonstrate regularization during hazard estimation. Since we are using logistic regression, existing packages, like \pkg{glmnet} can be leveraged. To demonstrate this feature, we examine the Study to Understand Prognoses Preferences Outcomes and Risks of Treatment (SUPPORT) dataset [@knaus1995support]. The SUPPORT dataset tracks death for individuals who are considered seriously ill within 5 hospitals in the United States of America. Imputation was conducted using the \pkg{mice} package in \proglang{R} with default settings [@mice]. The imputed dataset is available as part of the \pkg{casebase} package. For more information please see the SUPPORT data documentation in the package. 

we have a total of 9104 observations and 30 variables, two of which are SUPPORT day 3 physiology score (sps) and 	APACHE III day 3 physiology score (aps). These scores were developed for a period of 180 days, which makes our comparison over 5.56 years exploratory. Therefore, we will assess these two models before using either as our baseline, by comparing them to a full model using all variables. Also, our single imputation introduces bias in the data. As the goal here is purely demonstrative, this bias will be consistent with any of the following models. As such, their comparison is still valid, though any interpretation requires an awareness of said bias. <!--The SUPPORT dataset will be used to demonstrate regularization within casebase, while comparing their absolute risk predictions for a new individual.--> A description of used variables can be found in Table \ref{tab:support1} and a breakdown of each categorical variable in Table \ref{tab:support2}. The original data was obtained from the Department of Biostatistics at Vanderbilt University [@harrell_2020].
...
```{r baselines, echo=FALSE,eval=eval_cs3,fig.cap="\\label{fig:baselines} Absolute risk curves estimated from hazard models. Red: APS only hazard was used to generate the absolute risk. Green: All covariates aside from APS and SPS. Blue: SPS only. Purple: unadjusted Kaplan-Meier curve.",echo=FALSE}
...
```{r brierpt1, echo=FALSE,eval=eval_cs3,fig.cap="\\label{fig:brier1} Brier scores over survival time. Full model (Full) performs best, while the other two (APS only and SPS only) are comparable. Red: APS only model was used to generate the brier scores. Green: all covariates excluding APS and SPS. Blue: SPS only model.",echo=FALSE}
...
brierPlotter<-rbind(
      data.frame(brierScore = brierAPS$Brier$score$Brier[brierAPS$Brier$score$model == "PredictionModel"],times=times,model="APS"),
      data.frame(brierScore = brierFull$Brier$score$Brier[brierFull$Brier$score$model == "PredictionModel"],times=times,model="Full"),
            data.frame(brierScore = brierSPS$Brier$score$Brier[brierSPS$Brier$score$model == "PredictionModel"],times=times,model="SPS")
      )
...
To get the absolute risk following regularized cox regression, we had to work around limitations in existing packages. To make use of the \pkg{survival} package, we first get our hazard coefficients using \pkg{glmnet}'s coxnet implementation for lasso penalization [@regpathcox]. Next, we fit a cox model using the survival package with the selected variables only, from our previous fit. Finally, we replaced the coefficients in the survival object with those from our glmnet fit, allowing us to make use of the absolute risk tools available in the \pkg{survival} package.

```{r coxNetCode,eval=FALSE,echo=TRUE}
# hazard for cox using glmnet
coxNet <- glmnet::cv.glmnet(x = x, y = y, family = "cox", alpha = 1, standardize = TRUE)
#getting all non-zero covariates
nonzero_covariate_cox <- predict(coxNet, type = "nonzero", s = "lambda.1se")
#keeping coefficients for later
nonzero_coef_cox <- coef(coxNet, s = "lambda.1se")
# cutting dataset to only include nonzero covariates
cleanCoxData <- as.data.frame(cbind(y, x[, nonzero_covariate_cox]))
#create a skeleton object
coxNet <- survival::coxph(Surv(time = d.time, event = death) ~ ., data = cleanCoxData,x=TRUE)
#replace coefficients with regularized ones
coxNet$coefficients <- nonzero_coef_cox@x
```

For our penalized logistic regression model, we opted for the natural log of time, which assumes a Weibull distribution for our event. As casebase makes use of the glmnet package, we interact with the fitsmoothhazard.fit function using a matrix interface, where y contains the time and event variables and x contains all other variables we would like to include in the model. For the SUPPORT dataset, all factor variables were converted into dummy variables. Here, we used lasso penalization by setting alpha to 1, leading to our smooth hazard function.


 
```{r casebaseDisplayCode, eval=FALSE,echo=TRUE}
#regularized logistic regression to estimate hazard
cbFull <- casebase::fitSmoothHazard.fit(x, y, family = "glmnet",
                      time = "d.time", event = "death",
                      formula_time = ~ log(d.time), alpha = 1,
                      ratio = 100, standardize = TRUE, penalty.factor=c(0,rep(1,ncol(x))))
...
cbFull <- casebase::fitSmoothHazard.fit(x, y, family = "glmnet", time = "d.time", event = "death", formula_time = ~ log(d.time), alpha = 1, ratio = 10, standardize = TRUE,penalty.factor=c(0,rep(1,ncol(x))))
```
...
```{r coefplots,echo=FALSE, eval=eval_cs3, fig.cap="\\label{fig:cs3lolliPlot} A lollipop plot demonstrating the estimated coefficients between the following three models: Cox (green), penalized Cox (red), penalized logisitic (blue). If a coefficient is estimated to be 0, it is removed from the plot"}
...
dwplot(lolliplotDots)+
  theme(axis.text.y = element_text(size = 11, angle = 0, hjust = 1, vjust = 0),
  ...
  ggtitle("Coefficient plot")+
   labs(color = "Models")+
   scale_color_manual(values=q4[c(2,1,3)])
```

```{r include=TRUE,echo=FALSE,eval=eval_cs3,fig.cap="\\label{fig:cs3Final} Four absolute risk curves, integrated from the following hazard models: Cox (green), penalized Cox (red), penalized logistic (blue) and unadjusted (Kaplan-Meier, purple)."}
ggplot(as.data.frame(ab), aes(Time)) +
  ...
  ggtitle("Absolute risk curves")+
   scale_color_manual(values=q4[c(2,1,3,4)])
```
  


```{r riskregressionBrier,echo=FALSE,eval=eval_cs3,fig.cap="\\label{fig:cs3FinalBrier} 4 curves comparing the brier scores as they change over time between Cox regression (green), penalized Cox regression (red) and penalized logistic regression (blue) and a unadjusted curve (Kaplan-Meier, purple). Penalized logistic regression produced the lowest curve."}

class(cbFull) <- c("singleEventCB",class(cbFull))
brierFinalResults <- Score(list("Cox" = coxFull,"Penalized Cox"=coxNet,"Penalized Logistic"=cbFull), data = test, formula = Hist(time, event != 0) ~ 1, summary = NULL, se.fit = 0L, metrics = "brier", contrasts = FALSE, times =times)


levels(brierFinalResults$Brier$score$model)[levels(brierFinalResults$Brier$score$model)=="Null model"] <- "Unadjusted"
b=brierFinalResults

brierFinalResults$Brier$score$model<- factor(brierFinalResults$Brier$score$model, levels = c("Cox","Penalized Cox","Penalized Logistic", "Unadjusted"))
...
Our first comparison plot examines the covariates that were selected between all three hazard functions. Note that Cox regression without penalization selected more covariates than the other two, but only the intersection of cofficients with values greater than 0 for all models are displayed in \ref{fig:cs3lolliPlot}. A visible tendency is for Cox regression to have larger estimated coefficients. This is expected, as there are no restrictions on the size of coefficients. The smallest coefficient estimates are found in penalized logistic model, aside from cancer state (cano). These values do not inform us of model performance. To do so, we take our hazard functions and use them to predict the absolute risk of each individual in our test set, over time. The probabilites over time for each individual are averaged, resulting in the absolute risk curves in figure \ref{fig:cs3Final}. There is separation between the curves, but it is minimal.  To better demonstrate the differences between the models, we compare brier scores over time. \ref{fig:cs3FinalBrier} the Brier score is lower in our penalized logistic regression model, compared to the other models. This difference, however, is minimal. We can say with relative certainty that any of the hazard models outperform an unadjusted curve, while penalized logistic regression performs the best on this testing dataset.
