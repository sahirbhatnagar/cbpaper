...
abCB$Model <- "Penalized estimation"
...
reg_ci <- ggplot(ab, aes(Time)) +
  ...
  paper_gg_theme+ scale_x_continuous(labels=scaleFUNx,breaks = round(seq(0, 5, by = 1))) +scale_y_continuous(labels=scaleFUNy,n.breaks=9)
```
...
reg_brier <- ggplot(data = data_brier, mapping = aes(x = times, y = Brier, col = model)) +
  ...
  labs(color = "Models") +
  paper_gg_theme +  
  scale_color_manual(values = q4) + scale_x_continuous(labels=scaleFUNx,breaks = round(seq(0, 5, by = 1))) +scale_y_continuous(labels=scaleFUNy,n.breaks=8)
