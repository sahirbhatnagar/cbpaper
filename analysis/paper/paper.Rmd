...
newx <- model.matrix(death ~ . - d.time - aps - sps, 
                  data = test)[, -c(1)]
times <- sort(unique(test$d.time))
#times<-seq(from=min(times),to=max(times),length.out = 100)
...
estimatescb <- data.table::setDT(as.data.frame(coef(cbFull)[-c(1, 2), 1]), 
                     keep.rownames = TRUE)
# estimatescb$std.error<-0
estimatescb$Model <- "Pen. CB"
colnames(estimatescb) <- c("term", "estimate", "model")

estimatescoxFull <- data.table::setDT(as.data.frame(coef(coxFull)), 
                          keep.rownames = TRUE)
...
data_brier <- bind_rows(
  ...
  mutate(model = as.character(model), # First return to string, so types match
         model = case_when(
    model == "Null model" ~ "K-M",
    model == "PredictionModel" ~ "Cox",
    TRUE ~ model
  ),
  model = factor(model, levels = c("Cox", "Penalized Cox", "Penalized CB", "K-M"),
                 labels = c("Cox", "Pen. Cox", "Pen. CB", "K-M")))
...
As before, we can look at the population-time plot for a graphical summary of the event incidence. Here, we coloured the exposed person-time (i.e. after transplant) in a darker shade of gray. As we can see, most events occur early during the follow-up period, and therefore we do not expect the hazard to be constant.
...
Note that the third model includes an interaction term between exposure and follow-up time. In other words, this model no longer exhibit proportional hazards. The evidence of non-proportionality of hazards in the Stanford Heart Transplant data has been widely discussed [@arjas1988graphical].

We can then compare the goodness of fit of these three models using the Akaike Information Criterion (AIC).

```{r, eval = eval_cs4, dependson="stanford_fit"}
# Compute AIC
c("Model1" = AIC(fit1),
  "Model2" = AIC(fit2),
  "Model3" = AIC(fit3))
