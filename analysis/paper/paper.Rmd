...
The functions in the casebase package can be divided into two categories: 1) data visualization, in the form of population-time plots and hazard curves; and 2) parametric modeling. We explicitly aimed at being compatible with both \code{data.frame}s and \code{data.table}s. This is evident in some of the coding choices we made, and it is also reflected in our unit tests.
...
1. The \code{casebase::popTime} function takes as input the original dataset along with the column names corresponding to the timescale, the event status and an exposure group of interest (optional). This will create an object of class \code{popTime} (or \code{popTimeExposure} if you specify a value for the exposure argument).  
2. The corresponding \code{plot} methods for the object created in Step 1 can be called to create the population time plot with several options for customizing the aesthetics.  
...
To illustrate these functions we will use the European Randomized Study of Prostate Cancer Screening (ERSPC) data which is available through the \pkg{casebase} package. The results of this study were published by [@schroder2009screening], and the dataset itself was obtained using the approach described in [@liu2014recovering].

We first create the necessary dataset for producing the population time plots using the \code{casebase::popTime} function. In this example we stratify by the treatment group. Note that the resulting object stores the exposure variable as an attribute:

```{r plot-erspc-data, eval = eval_cs1, echo = TRUE}
data("ERSPC")
...
<!--The \code{pt_object} is a \code{data.table} containing the original dataset augmented with-->
...
```{r plot-stratified-erspc-data, eval = eval_cs1, echo = TRUE}
pacman::p_load(cowplot)
plot(pt_object, add.case.series = FALSE) + cowplot::theme_minimal_hgrid(12, rel_small = 1)
theme_cowplot
plot(pt_object, legend = TRUE, theme.params = list(legend.position = "bottom"))

# plot(pt_object, add.base.series = TRUE, ratio = 1, legend = TRUE)
```

Figure~\ref{fig:plot-stratified-erspc-data} is built sequentially by adding layers for the area representing the population time in grey, followed by points to show the case series. The case series are sampled at random vertically on the plot in order to visualise the incidence density using the \code{popTime} function. That is, imagine we draw a vertical line at a specific event time. We then plot the point at a randomly sampled y-coordinate along this vertical line. This is done to avoid having all points along the upper edge of the plot because the subjects with the least amount of observation time are plotted at the top of the y-axis. By randomly distributing the points, we can get a better sense of the inicidence density. In the ERSPC data, we can see that more events are observed later on in time. Therefore a constant hazard model would not be appropriate in this instance as it would overestimate the cumulative incidence earlier on in time, and underestimate it later on.



casebase sampling mechanism. This function gives user the flexibility to add any combination of the case series, base series and competing events.  The base series is sampled horizontally on the plot using the sampleCaseBase function.


The unique _step shape_ of the population time plot is due to the randomization of the Finnish cohorts which were carried out on January 1 of each of the 4 years 1996 to 1999. This, coupled with the uniform December 31 2006 censoring date, lead to large numbers of men with exactly 11, 10, 9 or 8 years of follow-up.




The randomization of the Finnish cohorts were carried out on January 1 of each of the 4 years 1996 to 1999. This, coupled with the uniform December 31 2006 censoring date, lead to large numbers of men with exactly 11, 10, 9 or 8 years of follow-up. Tracked backwards in time (i.e. from right to left), the population-time plot shows the recruitment pattern from its beginning in 1991, and in particular the Jan 1 entries in successive years. Tracked forwards in time (i.e. from left to right), the plot for the first three years shows attrition due entirely to death (mainly from other causes). Since the Swedish and Belgian centres were the last to close their recruitment - in December 2003 - the minimum potential follow-up is three years. Tracked further forwards in time (i.e. after year 3) the attrition is
...
For a more detailed description of how to modify the aesthetics of a population time plot, please refer to the [Customizing Population Time Plots](http://sahirbhatnagar.com/casebase/articles/customizingpopTime.html) vignette.
...
text(x = 15*0.97, y = 0.05/100, labels = "(A)", cex = 1.5)
...
text(x = 15*0.97, y = 0.05/100, labels = "(C)", cex = 1.5)
...
text(x = 15*0.97, y = 0.05/100, labels = "(B)", cex = 1.5)
...
text(x = 15*0.97, y = 0.05/100, labels = "(D)", cex = 1.5)
...
```{r erspc-casebase-cif, eval = eval_cs1, echo = echo_plot_code}
...
```{r erspc-casebase-weibull-cif, eval = eval_cs1, echo = echo_plot_code}
...
pt <- cbind(list("Exponential","Gompertz","Weibull","Splines"),
...
```{r erspc-casebase-splines-cif, eval = eval_cs1, echo = echo_plot_code}
...
kable(pt, "latex", booktabs = T, align = c("l","c","c"),
      caption = c("Mean "),
      col.names = c("Model","casebase","survreg")) %>%
  kable_styling() %>%
