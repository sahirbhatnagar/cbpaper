...
casebaseCI=casebase::absoluteRisk(cb.Model,time = seq(1,max(completeData$d.time), 20),newdata = x , s="lambda.1se",method=c("numerical"))

cbCIAve=data.frame(casebaseCI[,1],rowMeans(casebaseCI[,-c(1)]))
```
...
```
We used the survival package to calculate cox regression's cumulative incidience without penalization and the kaplan-meier curve for incidience.

```{r unpenalizedCox, echo = FALSE, eval = eval_cs3}
Cox<-as.data.frame(cbind(as.numeric(y[,2]),as.numeric(y[,1]),xCox[,-c(1)]))
...
plot(cbCIAve[,1],cbCIAve[,2],type='l',col="black",lwd=3,main="Cumulative Incidence comparison",xlab="Survival-Time",ylab="Cumulative Incidence",ylim=c(0,1),xlim=)
lines(abCF,col="red",fun="event",lwd=3,conf.int = FALSE)
lines(abKm ,col="Blue",fun="event",lwd=3,conf.int = FALSE)
lines(abCFup ,col="purple",fun="event",lwd=3,conf.int = FALSE)
legend("bottomright", 
       legend = c( "Penalized Weibull with casebase","Coxnet","Cox","Kaplan-Meier"), 
...
estimatescox=setDT(as.data.frame(coef(coxglmFit)[-c(1),1]), keep.rownames = TRUE)[]
